...
import { findSurvivingSecrets, type Recipe, type Tool } from '@douze/shared'
...
function loadFixture(tool: Tool, fixturesDir: string): Fixture | undefined {
  ...
  let fixture: Fixture
  try {
    fixture = JSON.parse(readFileSync(join(fixturesDir, reference), 'utf8')) as Fixture
  } catch {
    return undefined
  }
  ...
  if (leaked.length > 0) {
    throw new Error(
      `refusing to eject "${tool.name}": fixture "${reference}" carries a credential-shaped value at ${leaked.join(', ')}`,
    )
  }
  return fixture
}
