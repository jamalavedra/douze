 ...
 *
 *   RECON_TEST_ORIGIN=http://127.0.0.1:4180 pnpm --filter @recon/extension build
...
const userDataDir = mkdtempSync(join(tmpdir(), 'recon-smoke-'))
...
try {
  ...
  check(
    'manifest is the Recon MV3 manifest',
  ...
  await popup.waitForFunction(() => document.getElementById('connection')?.textContent !== '…')
  check('popup renders its heading', (await popup.textContent('h1'))?.includes('Recon'), true)
  check('popup shows the idle start form', await popup.isVisible('#start'), true)
  check('popup reports the daemon state', await popup.textContent('#connection'), 'daemon offline')
  check('popup defaults to the RECON_PORT convention', await popup.inputValue('#port'), '8787')
  ...
  check(
    ...
    await popup.evaluate(async () => {
      const status = await chrome.runtime.sendMessage({ type: 'recon:status' })
      return [status.session, status.count, status.connected]
    }),
    [null, 0, false],
  )
  check(
    'a session start with an empty name is rejected (AC-CAP-001.2)',
    await popup.evaluate(async () => {
      ...
      return typeof res.error === 'string' && res.error.includes('name')
    }),
    true,
  )
  ...
  if (!fixtureUp) {
  ...
  } else if (!(await worker.evaluate((o) => globalThis.__recon.hasOrigin(o), FIXTURE))) {
    console.log('SKIP  capture section — build without RECON_TEST_ORIGIN, host permission absent')
  } else {
    const app = await context.newPage()
    await app.goto(FIXTURE)
    // startSession reloads the tab itself: registrations do not reach an already-loaded tab.
    await worker.evaluate((o) => globalThis.__recon.startSession('orders', [o]), FIXTURE)
    await app.waitForSelector('#create')
    check(
      'interceptor is in the page after startSession reloaded it',
      await app.evaluate(() => window.__recon_interceptor__ === true),
      true,
    )
    ...
    check('badge reflects the captured count (AC-CAP-001.3)', await worker.evaluate(() => globalThis.__recon.badgeCount()), captured.length)

    await worker.evaluate(() => globalThis.__recon.annotate('creates an order'))
    const span = await worker.evaluate(() =>
      globalThis.__recon.pending().find((m) => m.type === 'exchange.annotate')?.span ?? null,
    )
    check('annotation spans the exchanges since the last note (AC-CAP-007.2)', [span?.note, span?.start_position], ['creates an order', 0])

    const stopped = await worker.evaluate(() => globalThis.__recon.stopSession())
    check('stop reports the retained count (AC-CAP-001.4)', stopped.retained, captured.length)
    check('badge clears on stop', await worker.evaluate(() => globalThis.__recon.badgeCount()), 0)
  }
} finally {
  await context.close()
