...
- [ ] T-015.10 — **NOT DONE (2026-08-11)**: the page is there from T-015.4 and the worker half is not. `onConnectCommand` in `packages/extension/src/background.ts` handles `douze:connect:pair` and `douze:connect:expose`; `start`, `rotate` and `stop` all return `NOT_CONNECTED_YET` ("Connecting to a hosted assistant is not wired up yet."), and nothing in the extension calls the relay's `POST /register`. `RelayPairing` and the `attach:relay` storage key exist and `attach.ts` re-reads them every tick, so this is the registration call and the state writes, not the plumbing.
