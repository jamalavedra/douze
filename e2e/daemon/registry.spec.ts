...
test.describe('COV_RUN_001: Registry loading', () => {
  ...
  test('@COV_RUN_001.1 should isolate a broken recipe from the rest', async () => {
    ...
    for (const name of ['alpha', 'beta']) {
      writeFileSync(join(douzed.home, 'recipes', `${name}.yaml`), recipe(name))
      writeFileSync(join(douzed.home, 'fixtures', `${name}.json`), '{"data":{"orders":[]}}')
    }
    writeFileSync(join(douzed.home, 'recipes', 'broken.yaml'), 'version: 1\nname: broken\ntools: notalist\n')
  ...
  test('@COV_RUN_001.2 should expose a fixture-invalid tool as degraded rather than omitting it', async () => {
    writeFileSync(join(douzed.home, 'recipes', 'alpha.yaml'), recipe('alpha'))
...
test.describe('COV_RUN_002: Hot reload', () => {
  ...
  test.beforeEach(async () => {
    douzed = new Douzed()
    writeFileSync(join(douzed.home, 'recipes', 'alpha.yaml'), recipe('alpha'))
    writeFileSync(join(douzed.home, 'fixtures', 'alpha.json'), '{"data":{"orders":[]}}')
    await douzed.start()
  })
  test.afterEach(() => douzed.stop())
  ...
  test('@COV_RUN_002.1 should pick up a description edit within 5 seconds', async () => {
    ...
    writeFileSync(join(douzed.home, 'recipes', 'alpha.yaml'), recipe('alpha', 'Returns open orders only.'))

    await expect
      .poll(
        async () => (await (await douzed.api('/registry')).json()).tools[0]?.tool.description,
  ...
  test('@COV_RUN_002.2 should keep serving the last valid version when a reload fails', async () => {
    writeFileSync(join(douzed.home, 'recipes', 'alpha.yaml'), 'version: 1\n  bad: [unclosed')

    await expect
      .poll(async () => (await (await douzed.api('/registry')).json()).errors.length, { timeout: 5_000 })
      .toBe(1)
  ...
  test('should stop serving a recipe that is deleted', async () => {
    rmSync(join(douzed.home, 'recipes', 'alpha.yaml'))
    await expect
      .poll(async () => (await (await douzed.api('/registry')).json()).tools.length, { timeout: 5_000 })
      .toBe(0)
  })
})
